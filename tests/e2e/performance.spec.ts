...
test.describe("large dataset performance", () => {
  test("imports 100,000 rows and keeps the mounted DOM bounded while scrolling", async ({
    ...
    try {
      execFileSync(process.execPath, ["tests/fixtures/generate-100k.mjs", csvPath], {
        stdio: "pipe",
      });
      ...
      await expect
        .poll(async () => page.locator("#coverageSummary").textContent(), {
          timeout: 120_000,
        })
        .toMatch(/^\d+\.\d{2}%$/);
      await expect(page.locator("#resultsList .mining-entry")).toHaveCount(50);
      ...
      await expect(page.locator("#resultStats")).toContainText("99,800 currently shown", {
        timeout: 60_000,
      });
      await expect(page.locator("#resultsList .vl-spacer-bottom")).toHaveCount(1, {
        timeout: 60_000,
      });
