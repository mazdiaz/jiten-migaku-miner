...
import { dispatchStoreOperation, StoreError } from "../../../server/store";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const result = await dispatchStoreOperation(input);
    ...
    return Response.json(
      { error: "Database operation could not be confirmed. Reload to check your saved data." },
      { status: 503 },
    );
  }
}
